...
- "Which state had the largest percent increase in Cigna visits from 2023 to 2024?"
MATCH (h:Hospital)<-[:AT]-(v:Visit)-[:COVERED_BY]->(p:Payer)
WHERE p.name = 'Cigna' AND v.admission_date >= '2023-01-01' AND
v.admission_date < '2025-01-01'
WITH h.state_name AS state, COUNT(v) AS visit_count,
     SUM(CASE WHEN v.admission_date >= '2023-01-01' AND
     v.admission_date < '2024-01-01' THEN 1 ELSE 0 END) AS count_2023,
     SUM(CASE WHEN v.admission_date >= '2024-01-01' AND
     v.admission_date < '2025-01-01' THEN 1 ELSE 0 END) AS count_2024
WITH state, visit_count, count_2023, count_2024,
     (toFloat(count_2024) - toFloat(count_2023)) / toFloat(count_2023) * 100
     AS percent_increase
...
REVIEW_GENERATION_TEMPLATE = """Your job is to use patient
